import { AutoCompleteEx, Button, Form, Icon, injectContext, Input, Label, Modal, SelectEx, SelectItem, useAjax, widgetFactory } from 'didgah/ant-core-component';
...
function CascadeDropdownInGrid(props: ComponentProps<AggregationOneToOneValue> & { referenceDataModelGuid: string; CascadeDropDownLevel: number, Widget: WidgetType }) {
    ...
    const handleChange = (value, level, index) => {
        ...
        if (!!value) {
            ...
            if (index === dataModels.length - 1) {
                ...
                if (index === CascadeDropDownLevel - 1) setValue(selectedItem.key)
    ...
    return <>
        {props.Widget !== WidgetType.DisabledWidget && <div style={{ display: 'flex', alignItems: 'center' }}>
            <div style={{ flexGrow: 4 }}><Label>{value}</Label></div>
            <Icon type='ellipsis' style={{ fontWeight: 'bold', fontSize: '20px', flexGrow: 1 }} onClick={handleButtonClick} />
        </div>}
        ...
        {visible && (
            ...
            >
                <>{dataModels.map((dsi, index) => (
                    <Form.Item label={dsi.title}>
                        <SelectEx
                            key={index}
                            dataSource={values[index]?.dropDownData ?? []}
                            onSelect={(value, option) => handleSelect(value, option, index)}
                            disabled={props.mode === 'design' ? levelInfo[dsi.level].disabled : props.value ? !props.value : levelInfo[dsi.level].disabled}
...
export default CascadeDropdownInGrid
