...
import { ArchiveLayoutDesignerViewModel, ArchiveLayoutSimpleDesignerViewModel, DefineLayoutDesignerViewModel, DefineLayoutSimpleDesignerViewModel, LayoutsModel, SimpleDesignerIStateStack, SimpleDesignerLayoutViewModelWithState } from "../../../typings/Core.DynamicDataModel/Types";
...
interface IStateHelper {
    getNewFloor: () => SimpleDesignerIStateStack<SimpleDesignerLayoutViewModelWithState>;
}
...
const newDefineFloor = (params: StateManagerParams): SimpleDesignerIStateStack<DefineLayoutSimpleDesignerViewModel> => {
    return getStateStackObj(params);
}

const newArchiveFloor = (params: StateManagerParams): SimpleDesignerIStateStack<ArchiveLayoutSimpleDesignerViewModel> => {
    return getStateStackObj(params);
}

const newDefineArchiveFloor = <T>(params: StateManagerParams): SimpleDesignerIStateStack<T> => {
    return getStateStackObj<T>(params);
}

const newInlineArchiveFloor = <T>(params: StateManagerParams): SimpleDesignerIStateStack<T> => {
    return getStateStackObj<T>(params);
}

const getStateStackObj = <T>(params: StateManagerParams): SimpleDesignerIStateStack<T> => {
