...
import {
  ...
  LayoutItemNonBindableSimpleDesignerViewModel,
...
export const SubLayoutContext = React.createContext({
  subLayoutGuid: null
});
...
// export const subLayoutGuid = utility.newGuid();
const LayoutDesignerPane = (props: LayoutDesignerPaneProps) => {
  const subLayoutGuid = React.useRef(utility.newGuid()).current;
  const initialLocalState = React.useRef(getInitialLocalState(subLayoutGuid));
  ...
  const focusOverlayCSS = PropertiesDockLayoutState.IsFormFocused || PropertiesDockItemState.FocusedLayoutItemGuid
  ...
  const handleAddFieldset = () => {
    const NewGroupItem: LayoutItemNonBindableSimpleDesignerViewModel = {
      Guid: utility.newGuid(),
      ParentGuid: null,
      Type: LayoutItemType.NoneBindable,
      Design: JSON.stringify({
