...
import { Form, FormItem, Modal, WrappedFormUtils } from "didgah/ant-core-component";
import {
...
import { isExistLayoutitemInCondition, isExistLayoutitemInEvent } from "../../../SimpleDesigner/services/widgetManager";
...
export const Field = (props: FieldProps) => {
  ...
  const localProps = React.useContext(LocalPropsContext);
  ...
  const [Component, setComponent] = React.useState<ComponentModel>(null);
  ...
  const getOptionBar = (layoutItem: LayoutItemType) => {
    const ColumnActionFramOptions = true ? [
            {
      ...
      switch (column.Type) {
        ...
        default:
          const newComponentModel = {
            ...widgetFactory.getComponent(column.LayoutItemGuid, true),
          };
  ...
  if (!!Component && !!Setting) {
    ...
    if (Component.layoutItemType === LayoutItemType.Reference) {
      rest.referenceDataModelGuid = column.LayoutItemGuid ? (widgetFactory.getLayoutItem(column.LayoutItemGuid) as RichLayoutItem).simpleDesignerData.DataModelInfo.ReferenceDataModelGuid : null
    }
    ...
    return (
      ...
      >
        <div style={{ clear: "both", color: 'red !important' }}>
