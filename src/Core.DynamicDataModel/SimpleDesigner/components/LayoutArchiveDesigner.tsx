...
const LayoutArchiveDesigner = () => {
  const globalProps = React.useContext(SimpleDesignerGlobalPropsContext);
  const { onSaveForm, closeWindow, layoutType, layoutGuid, mode, permission } = globalProps;
    ...
    <Spin spinning={IsLoading} stretch>
      ...
      >
        <TabPane key={LayoutTabs.Designer.toString()} tab={translate("Design")}>
          {!IsLoading && <LayoutArchive layoutType={LayoutType.Archive} />}
        </TabPane>
...
export default LayoutArchiveDesigner;
