...
function QueryBuilderActionConditionEditor({ form, onSubmit, onCancel, field, operator, extraData, layoutItemDataSource }: Props & FormComponentProps) {
  ...
  //consoleProxy("layoutItemDataSource",layoutItemDataSource)
 // const dd=layoutItemDataSource.filter(item => (item as any).Metadata.Type === LayoutItemType.Column).map((l: any) => ({ key: (l as any).Text, value: l.Id}))}

  //console.log(dd)
  //consoleProxy2("layoutItemDataSource",dd)
