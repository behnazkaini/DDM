...
import { IDictionary, LayoutItemReferenceSetting, ReferenceTokenContainer, RichLayoutItem, SettingFormItemProps } from "../../../../typings/Core.DynamicDataModel/Types";
...
export interface RefrenceTokenContainerColumnsProps {
  initialSettingValues: LayoutItemReferenceSetting;
  settingName: string;
...
const RefrenceTokenContainerColumns = (
  props: SettingFormItemProps<ReferenceTokenContainer> & { layoutItem: LayoutItemViewModel, isSimpleDesignerMode: boolean }
) => {
  ...
  const WrapperInput = (args: ViewModel) => {
    const [errors, setErrors] = React.useState<Array<string>>([]);
    const { onChange, value, columns } = args;
    const store1 = React.useRef(new CheckboxListStore()).current;
    const checkBoxItems = columns.map((column) => {
    ...
    const handleOnChangeCheckbox = (value: CheckboxItem) => {
      ...
      if (value.selected === true) {
        ...
        const newColumnTags = selectedColumnsTag.filter(
          (tag) => tag.value.toLowerCase() !== value.value.toLowerCase()
        );

        setSelectedColumnsTag(newColumnTags);
      }
    ...
    const makePreviewValue = (columns: CheckboxItem[]) => {
      let labelTokenContainer = "";

      if (columns == null) {
        return labelTokenContainer;
      }

      columns.forEach((column) => {
        labelTokenContainer += ` ${seperableCharacter} [${column.text}]`;
      });

      return labelTokenContainer.substring(2);
    };
    ...
    const tokenContainerPreview = makePreviewValue(selectedColumnsTag);

    return (
      <div key={key}>
        <StackPanel verticalMode={false}>
          <Label>{tokenContainerPreview}</Label>
          <Button
            type="ghost"
            icon="ellipsis"
  ...
  React.useEffect(() => {
    if (isSimpleDesignerMode) {
      getDataModel({ Guid: (layoutItem as RichLayoutItem).simpleDesignerData.ReferenceDataModelGuid }).then((r) => {
        setDataModel(r.find(d => d.Guid === (layoutItem as RichLayoutItem).simpleDesignerData.ReferenceDataModelGuid));
      })
    }
  }, []);
  ...
  return dataModel ? (
    ...
    >
      {form.getFieldDecorator(settingName, {
        initialValue:
          initialSettingValues.ReferenceTokenContainerColumnsConfige,
      })(<WrapperInput columns={dataModel.Columns} />)}
    </Form.Item>
  ) : <Spin />;
};

export default RefrenceTokenContainerColumns;
