...
import { errorMesseage } from "../../../Designer/services/widgetManager";
import {
  GlobalPropsContext,
  LocalPropsContext,
...
const SignComponents = (props: { Options: SignList }) => {
...
const ActionBarFrame = (props: ActionBarFrameProps) => {
  const { permission } = React.useContext(GlobalPropsContext);
  const {
    Options,
    style,
    designError,
  ...
  const hiddenStyle= props.isNotFocus ? "DDM-container-hidden" : undefined;
  ...
  return (
    <SortableBox
      ...
      type={type}
    >
      <div className={` ${columnStyle} ${focusStyle} ${hiddenStyle}`} >
        <ul>
          {!permission.readOnly && (
            <OptionComponents
              Options={Options}
