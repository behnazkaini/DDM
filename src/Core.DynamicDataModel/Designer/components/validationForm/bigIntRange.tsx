...
interface BigIntegerRangeSettingProps {
...
const BigIntegerRangeSetting = (props: BigIntegerRangeSettingProps) => {
  const { initialValue, form, errorCallback } = props;
  const { Min, Max } = initialValue;
  const maxIntNumber = 9223372036854775807;
  const minIntNumber = -9223372036854775808;

  const [value, setValue] = React.useState<RangeValidationSetting<number>>({
    Min: Min,
    Max: Max,
  });
  ...
  const checkLengthValidation = (
    ...
    if (value.Min != null && value.Min < minIntNumber) {
      return false;
    }

    if (value.Max != null && value.Max > maxIntNumber) {
      return false;
    }
        ...
        <Form.Row>
          ...
          >
            <NumericalInput
              ...
              doNotConvertValueToNumber={false}
...
export default BigIntegerRangeSetting;
