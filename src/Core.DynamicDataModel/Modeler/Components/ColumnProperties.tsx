...
import { Alert, Button, Checkbox, Form, FormComponentProps, FormLayout, Input, InputNumber, SelectEx } from 'didgah/ant-core-component';
...
const intigerBookmarkType=[{ key: translate('InputNumber')  , value: 0 },{ key: translate('InputNumberWithSeperator')  , value: 1 }]
const decimalBookmarkType=[{ key: translate('InputNumber')  , value: 0 },{ key: translate('InputDecimalWithSeperator')  , value: 1 }]

function ColumnProperties({ form, selectedItem, onColumnPropertyChange, hostSoftwareGuid, table, inline = true, useFormLayoutWrapper = true, availableDataTypes }: Props) {
	...
	useEffect(() => {
		...
		setFieldsValue({ bookmarkType: selectedItem.bookmarkType });
		form.validateFields(() => { });
	...
	const InnerForm = (
		<Form>
          ...
          case ColumnDataType.BigInteger:
						return (
							<>
								<FormItem label={translate('ShowBookmarkType')} {...formItemSize}>
                  {getFieldDecorator('bookmarkType',{initialValue:0})(
                    <SelectEx dataSource={intigerBookmarkType} onChange={(value) => { handleFieldChange('bookmarkType', value) }}  />
                  )}
                </FormItem>
							</>
						)
              ...
              <FormItem label={translate('ShowBookmarkType')} {...formItemSize}>
                  {getFieldDecorator('bookmarkType',{initialValue:0})(
                    <SelectEx dataSource={decimalBookmarkType} onChange={(value) => { handleFieldChange('bookmarkType', value) }}  />
                  )}
                </FormItem>
								<FormItem label={translate('DDMModelerPrecision')} {...settingFormItemSize} validateStatus={precisionAndScaleFormAlert ? 'error' : 'success'}>
									{getFieldDecorator('precision', { initialValue: 18, rules: [{ required: true }], })(
								...
								<FormItem label={translate('DDMModelerScale')} {...settingFormItemSize} validateStatus={precisionAndScaleFormAlert ? 'error' : 'success'} >
									{getFieldDecorator('scale', { initialValue: 0, rules: [{ required: true }] })(
