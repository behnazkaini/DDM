...
import { Controls } from "../../../../../../../../../Web/Junction/DidgahShared/Development/Didgah5/Core.Common/Scripts/Models/Chargoon.Didgah.Common.Domain.Enumeration.Controls";
...
function SignatureSelectorOneToOne({ value = emptyObj, onChange, context, initValue, Disabled = false, primaryKey, dataModelGuid, DefaultValue, resetForm, Widget }: SignatureSelectorOneToOneProps) {
	...
	const openSign = async () => {
		setLoading(true);
		const userSignatureSetting: UserSignatureGuidSetting = await API_Model.GetSignatureSettings()
		if (userSignatureSetting.HasSignature) {
			let signatureGuid;
			if (userSignatureSetting.MustSelectSignature) {
				const selectedSignatureId = await context.commandHandler.openControlFormByCode({
